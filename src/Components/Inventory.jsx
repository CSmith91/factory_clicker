                ...
                {Object.entries(ingredients)
                    ...
                    .map(([ingredientName, ingredientData]) => (
                        <div style={{display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
                            ...
                            <li key={ingredientName} style={{marginLeft: '10px'}}>{ingredientName}: {ingredientData.count} ----- Idle Count: {ingredientData.idleCount}</li>
                        </div>
                    ))}
            </ul>
            ...
            <ul style={{ listStyleType: 'none', padding: 0 }}>
                {Object.entries(ingredients)
                    ...
                    .map(([ingredientName, ingredientData]) => (
                        <div style={{display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
                            {ingredientData.isCraftable && <button>Craft</button>}
                            <li key={ingredientName} style={{marginLeft: '10px'}}>{ingredientName}: {ingredientData.count}</li>
                        </div>
                    ))}
            </ul>
        </div>
